...
class Network:
    ...
    def copy_trainables_from_gpu_network(self, src_net):
        gpu_values = run({name: src_net.find_var(name) for name in src_net.trainables.keys()})
        cpu_values = {name.replace('G_paper','G_paper_cpu'): gpu_values[name] for name in gpu_values.keys()}
#       for name in gpu_values.keys():
#            if "weight" in name:
#                name_to_value[name] = tf.get_default_session().run(tf.transpose(name_to_value[name], [0, 2, 3, 1]))
        set_vars({self.find_var(name): value for name, value in cpu_values.items()})
    ...
    def run(self, *in_arrays,
        ...
        num_cpus        = 1,        # Number of GPUs to use.
        ...
        **dynamic_kwargs):          # Additional keyword arguments to pass into the network construction function.
        ...
        key = str([list(sorted(dynamic_kwargs.items())), num_cpus, out_mul, out_add, out_shrink, out_dtype])
            ...
            with absolute_name_scope(self.scope + '/Run'), tf.control_dependencies(None):
                ...
                for cpu in range(num_cpus):
                    with tf.device('/cpu:%d' % cpu):
                        out_expr = self.get_output_for(*in_split[cpu], return_as_list=True, **dynamic_kwargs)
                        if out_mul != 1.0:
                            out_expr = [x * out_mul for x in out_expr]
                        if out_add != 0.0:
                            out_expr = [x + out_add for x in out_expr]
                        if out_shrink > 1:
                            ksize = [1, out_shrink, out_shrink, 1]
                            out_expr = [tf.nn.avg_pool(x, ksize=ksize, strides=ksize, padding='VALID', data_format='NHWC') for x in out_expr]
